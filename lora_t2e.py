...
class EmotionFineTuner:
    def __init__(self, model_name="Qwen/Qwen3-1.7B", lora_rank=8, lora_alpha=32, vocab_file="checkpoints/bpe.model"):
        ...
        # 设置 pad_token，使用 eos_token 作为 pad_token
        # 注意：TextTokenizer 的 pad_token 为 None，所以我们用 eos_token 代替
    ...
    def prepare_data(self, dataset_path):
        ...
        """
        # 假设你的数据集格式为: [{"text": "...", "emotion_probs": [0.1, 0.2, ...]}]
        with open(dataset_path, 'r', encoding='utf-8') as f:
            data = json.load(f)

        texts = [item["text"] for item in data]
        emotion_probs = [item["emotion_probs"] for item in data]
        ...
        for seq in batch_encoded:
            ...
            if len(seq) > max_length:
                ...
                padded_seq = seq + [self.tokenizer.eos_token_id] * (max_length - len(seq))
...
def main():
    ...
    # 准备数据（假设你已经有数据集）
    # dataset = tuner.prepare_data("your_dataset.json")
