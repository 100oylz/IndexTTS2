...
class IndexTTS2:
    def __init__(
        ...
        if device is not None:
            ...
            self.use_cuda_kernel = use_cuda_kernel is not None and use_cuda_kernel and device.startswith(
                "cuda")
        ...
        self.qwen_emo = QwenEmotion(os.path.join(
            self.model_dir, self.cfg.qwen_emo_path))
        ...
        if self.use_cuda_kernel:
            ...
            try:
                ...
                print(">> Preload custom CUDA kernel for BigVGAN",
        ...
        print('>> semantic_codec weights restored from: {}'.format(
            semantic_code_ckpt))
        ...
        emo_matrix = torch.load(os.path.join(
            self.model_dir, self.cfg.emo_matrix))
        self.emo_matrix = emo_matrix.to(self.device)
        self.emo_num = list(self.cfg.emo_num)

        spk_matrix = torch.load(os.path.join(
            self.model_dir, self.cfg.spk_matrix))
        ...
        self.model_version = self.cfg.version if hasattr(
            self.cfg, "version") else None
        ...
        for i in range(0, codes.shape[0]):
            ...
            if not torch.any(code == self.stop_mel_token).item():
                ...
                stop_mel_idx = (code == self.stop_mel_token).nonzero(
                    as_tuple=False)
                len_ = stop_mel_idx[0].item() if len(
                    stop_mel_idx) > 0 else code.size(0)
    ...
    def normalize_emo_vec(self, emo_vector, apply_bias=True):
        ...
        if apply_bias:
            ...
            emo_bias = [0.9375, 0.875, 1.0, 1.0,
                        0.9375, 0.9375, 0.6875, 0.5625]
            emo_vector = [vec * bias for vec,
                          bias in zip(emo_vector, emo_bias)]
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path,
        ...
        if emo_vector is not None:
            ...
            if emo_vector_scale != 1.0:
                ...
                print(
                    f"scaled emotion vectors to {emo_vector_scale}x: {emo_vector}")
        ...
        if self.cache_spk_cond is None or self.cache_spk_audio_prompt != spk_audio_prompt:
            ...
            ref_target_lengths = torch.LongTensor(
                [ref_mel.size(2)]).to(ref_mel.device)
            ...
            style = self.campplus_model(
                feat.unsqueeze(0))  # 参考音频的全局style2[1,192]
        ...
        if emo_vector is not None:
            ...
            if use_random:
                ...
                random_index = [find_most_similar_cosine(
                    style, tmp) for tmp in self.spk_matrix]

            emo_matrix = [tmp[index].unsqueeze(
                0) for index, tmp in zip(random_index, self.emo_matrix)]
        ...
        if self.tokenizer.unk_token_id in text_token_ids:
            ...
            print("     Tokens which can't be encoded: ", [t for t, id in zip(
                text_tokens_list, text_token_ids) if id == self.tokenizer.unk_token_id])
            print(
        ...
        for seg_idx, sent in enumerate(segments):
            ...
            text_tokens = torch.tensor(
                text_tokens, dtype=torch.int32, device=self.device).unsqueeze(0)
            if verbose:
                print(text_tokens)
                print(
                ...
                print("text_token_syms is same as segment tokens",
            ...
            with torch.no_grad():
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    ...
                    if emo_vector is not None:
                        emovec = emovec_mat + \
                            (1 - torch.sum(weight_vector)) * emovec
                        # emovec = emovec_mat
                ...
                m_start_time = time.perf_counter()
                use_speed = torch.zeros(spk_cond_emb.size(0)).to(
                    spk_cond_emb.device).long()
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    latent = self.gpt(
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=dtype is not None, dtype=dtype):
                    ...
                    m_start_time = time.perf_counter()
                    wav = self.bigvgan(
                        vc_target.float()).squeeze().unsqueeze(0)
                    print(wav.shape)
                ...
                if verbose:
                    print(f"wav shape: {wav.shape}", "min:",
                          wav.min(), "max:", wav.max())
                # wavs.append(wav[:, :-512])
...
class QwenEmotion:
    def __init__(self, model_dir):
        ...
        self.desired_vector_order = ["高兴", "愤怒",
                                     "悲伤", "恐惧", "反感", "低落", "惊讶", "自然"]
    ...
    def convert(self, content):
        ...
        emotion_dict = {
            self.cn_key_to_en[cn_key]: self.clamp_score(
                content.get(cn_key, 0.0))
    ...
    def inference(self, text_input):
        ...
        if any(word in text_input_lower for word in self.melancholic_words):
            ...
            content["悲伤"], content["低落"] = content.get(
                "低落", 0.0), content.get("悲伤", 0.0)
...
if __name__ == "__main__":
    ...
    tts = IndexTTS2(cfg_path="checkpoints/config.yaml",
                    model_dir="checkpoints", use_cuda_kernel=False)
    tts.infer(spk_audio_prompt=prompt_wav, text=text,
