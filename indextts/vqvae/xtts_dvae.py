...
# 默认值处理函数
def default(val, d):
    return val if val is not None else d# 如果val不为None则返回val，否则返回d

# 模型评估装饰器
def eval_decorator(fn):
    def inner(model, *args, **kwargs):
        was_training = model.training# 保存模型原始训练状态
        model.eval()# 切换到评估模式
        out = fn(model, *args, **kwargs)# 执行被装饰函数
        model.train(was_training)# 恢复原始训练状态
        return out
